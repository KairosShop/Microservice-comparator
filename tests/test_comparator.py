...
from context import some_prod, no_prod, one_same_prod, one_dif_prod
...
class MainTest(TestCase):
    ...
    def test_full_products(self):
        ...
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, full_prod()]}
    ...
    def test_half_products(self):
        ...
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, half_prod()]}
        response = self.client.post(url_for('comparator'), json=fake_data)
        self.assertEquals(response.json, res_half_prod())

    def test_one_market_all_products(self):
        """Test one supermarket with all products."""
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, one_market_all_prod()]}
        response = self.client.post(url_for('comparator'), json=fake_data)
    ...
    def test_two_markets_all_products(self):
        ...
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, two_markets_all_prod()]}
        response = self.client.post(url_for('comparator'), json=fake_data)
    ...
    def test_some_products(self):
        ...
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, some_prod()]}
        response = self.client.post(url_for('comparator'), json=fake_data)
        self.assertEquals(response.json, res_some_prod())

    def test_no_products(self):
        """Test all supermarkets with no propducts."""
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, no_prod()]}
        response = self.client.post(url_for('comparator'), json=fake_data)
    ...
    def test_one_same_product(self):
        ...
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, one_same_prod()]}
        response = self.client.post(url_for('comparator'), json=fake_data)
    ...
    def test_one_dif_product(self):
        ...
        fake_data = {'cart': [self.user_info, self.markets_id, self.markets_loc, self.products, one_dif_prod()]}
        response = self.client.post(url_for('comparator'), json=fake_data)
